...
export default function ShareBillModal({ isOpen, onClose, invoiceId = "INV-20251215-001" }: Props) {
  if (!isOpen) return null;

  const shareUrl = `https://example.com/invoice/${invoiceId}`;
  ...
  const handleEmail = () => {
    window.open(`mailto:?subject=Invoice&body=${encodeURIComponent(shareUrl)}`, "_blank");
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-8 shadow-2xl shadow-black/50 mx-4">

        <div className="flex items-start justify-between mb-1">
          <h2 className="text-base font-semibold text-slate-800">Share this Invoice</h2>
          <button onClick={onClose} className="text-slate-400 hover:text-slate-600 transition mt-0.5">
            <X size={18} />
          </button>
        </div>
        <p className="text-xs text-slate-400 mb-8">Share this Invoice via WhatsApp, Email, or more.</p>
        ...
        <div className="flex items-center justify-center gap-9 mb-8">
          ...
          {/* Email */}
          <button onClick={handleEmail} className="flex flex-col items-center gap-1.5 group">
            <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center group-hover:bg-slate-200 transition">
              <svg viewBox="0 0 24 24" className="w-6 h-6 text-slate-600 stroke-current fill-none" strokeWidth={1.8}>
          ...
          <button onClick={() => window.print()} className="flex flex-col items-center gap-1.5 group">
            <div className="w-12 h-12 rounded-full bg-blue-50 flex items-center justify-center group-hover:bg-blue-100 transition">
              <svg viewBox="0 0 24 24" className="w-6 h-6 text-blue-500 stroke-current fill-none" strokeWidth={1.8}>
                <path d="M6 9V3h12v6" />
        ...
        <div className="flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2.5">
          ...
          <button onClick={handleCopy} className="text-slate-400 hover:text-slate-600 transition shrink-0">
            <Copy size={16} />
          </button>
        </div>
